...
export const NotesLibrary: React.FC<NotesLibraryProps> = ({ notes, onOpenNote, onCreateNote }) => {
  ...
  const filteredNotes = useMemo(
    () =>
      notes.filter((note) => {
        const matchesQuery = `${note.title} ${note.content.map((block) => block.content).join(' ')}`
          .toLowerCase()
          .includes(query.toLowerCase())
        return (
          matchesQuery &&
          (subject === 'all' || note.subject === subject) &&
          (color === 'all' || note.color === color)
        )
      }),
    ...
    <section className={styles.page} aria-labelledby="notes-library-title">
      <header className={styles.header}>
        <div>
          ...
          <h1 id="notes-library-title" className={styles.title}>
            My Notes
          </h1>
      ...
      {filteredNotes.length > 0 ? (
        ...
        <div className={styles.empty}>
          ...
          <button type="button" onClick={onCreateNote} className={styles.newButton}>
