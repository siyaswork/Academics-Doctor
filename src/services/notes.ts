...
export async function listNotes() {
  ...
  const { data, error } = await supabase.from<DBNote>('notes').select('*').eq('user_id', userId).order('created_at', { ascending: false })
...
export async function createNote(payload: Partial<DBNote>) {
  ...
  const { data, error } = await supabase.from<DBNote>('notes').insert(insert).select().maybeSingle()
  return { data, error }
}
...
export async function getNoteWithBlocks(noteId: string) {
  ...
  const { data: note, error: noteError } = await supabase.from<DBNote>('notes').select('*').eq('id', noteId).eq('user_id', userId).maybeSingle()
  ...
  const { data: blocks, error: blocksError } = await supabase.from<DBNoteBlock>('note_blocks').select('*').eq('note_id', noteId).eq('user_id', userId).order('position', { ascending: true })
...
export async function updateNote(noteId: string, updates: Partial<DBNote>) {
  const userId = await getCurrentUserId()
  if (!userId) return { data: null, error: new Error('Not authenticated') }
  const { data, error } = await supabase.from<DBNote>('notes').update(updates).eq('id', noteId).eq('user_id', userId).select().maybeSingle()
  return { data, error }
}
...
export async function deleteNote(noteId: string) {
  const userId = await getCurrentUserId()
  if (!userId) return { data: null, error: new Error('Not authenticated') }
  const { data, error } = await supabase.from<DBNote>('notes').delete().eq('id', noteId).eq('user_id', userId)
  return { data, error }
}
...
export async function createNoteBlock(noteId: string, block: Partial<DBNoteBlock>) {
  ...
  const { data, error } = await supabase.from<DBNoteBlock>('note_blocks').insert(payload).select().maybeSingle()
  return { data, error }
}
...
export async function updateNoteBlock(blockId: string, updates: Partial<DBNoteBlock>) {
  const userId = await getCurrentUserId()
  if (!userId) return { data: null, error: new Error('Not authenticated') }
  const { data, error } = await supabase.from<DBNoteBlock>('note_blocks').update(updates).eq('id', blockId).eq('user_id', userId).select().maybeSingle()
  return { data, error }
}
...
export async function deleteNoteBlock(blockId: string) {
  const userId = await getCurrentUserId()
  if (!userId) return { data: null, error: new Error('Not authenticated') }
  const { data, error } = await supabase.from<DBNoteBlock>('note_blocks').delete().eq('id', blockId).eq('user_id', userId)
  return { data, error }
}
...
export async function replaceBlocksForNote(noteId: string, content: RichTextContent[]) {
  ...
  const del = await supabase.from<DBNoteBlock>('note_blocks').delete().eq('note_id', noteId).eq('user_id', userId)
  ...
  const { data, error } = await supabase.from<DBNoteBlock>('note_blocks').insert(inserts).select()
  return { data, error }
}
