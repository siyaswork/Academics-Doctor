...
export async function startSession(payload: Partial<StudySession>) {
  ...
  const { data, error } = await supabase.from<StudySession>('study_sessions').insert(s).select().maybeSingle()
  return { data, error }
}
...
export async function endSession(sessionId: string) {
  ...
  const { data: session, error: getErr } = await supabase.from<StudySession>('study_sessions').select('*').eq('id', sessionId).eq('user_id', userId).maybeSingle()
  ...
  const { data, error } = await supabase.from<StudySession>('study_sessions').update({ ended_at, duration }).eq('id', sessionId).eq('user_id', userId).select().maybeSingle()
  return { data, error }
}
...
export async function listStudySessions() {
  ...
  const { data, error } = await supabase.from<StudySession>('study_sessions').select('*').eq('user_id', userId).order('created_at', { ascending: false })
  return { data, error }
}
